...
def get_current_datetime():
    ...
    fuso_horario_brasilia = pytz.timezone('Europe/Moscow')
...
def add_new_board(board_uri, board_name, board_description, username, captcha_input, captcha_text):
    ...
    all_boards = DB.find_all('boards')
    if not all_boards:
        # Если досок нет, начинаем с ID 0
        new_board_id = 0
    else:
        # Находим максимальный существующий ID и прибавляем 1
        max_existing_id = max(board['id'] for board in all_boards)
        new_board_id = max_existing_id + 1

    new_board = {
        'id': new_board_id, # Используем новый, правильный ID
...
def add_new_reply(user_ip, reply_to, post_name, comment, embed, files, filesthb):
    ...
    new_reply = {
        ...
        'post_date': get_current_datetime(),
        'content': comment,
        'imagesthb': filesthb  ,# Now stores a list of files
...
def remove_post(post_id):
    """Remove a post, all its replies, and all associated media files."""
    def delete_media_files(file_list, base_path):
    ...
    # Remove from pinned posts if exists
    ...
    for reply in replies:
        ...
        # Delete reply thumbnails
        delete_media_files(reply.get('imagesthb', []), './static/reply_images/')
...
def remove_reply(reply_id):
    """Remove a reply and all its associated images."""
    reply = DB.query('replies', {'reply_id': {'==': reply_id}})
    if not reply:
        return False

    reply_data = reply[0]  # Получаем данные ответа
    
    # Удаляем основные изображения
    if 'images' in reply_data and isinstance(reply_data['images'], list):
        for image in reply_data['images']:
            try:
                image_path = os.path.join('./static/reply_images/', image)
                if os.path.exists(image_path):
                    os.remove(image_path)
            except Exception as e:
                print(f"Error deleting image {image}: {e}")

    # Удаляем thumbnail изображения
    if 'imagesthb' in reply_data and isinstance(reply_data['imagesthb'], list):
        for thumb in reply_data['imagesthb']:
            try:
                thumb_path = os.path.join('./static/reply_images/', thumb)  # Путь уже включает 'thumbs/'
                if os.path.exists(thumb_path):
                    os.remove(thumb_path)
            except Exception as e:
                print(f"Error deleting thumbnail {thumb}: {e}")
...
def pin_post(post_id):
    ...
    new_pinned = {
        ...
        'imagesthb': post.get('imagesthb', [post.get('imagesthb', '')]),
